...
Resolution decides which version of which package ends up in the graph. Every field below lives in `package.json`, and every one is a field npm, pnpm, Yarn, or Bun already reads — each links out to its canonical documentation.

### `overrides`

Force one version of a package everywhere in the graph, whoever asked for it. A selector is a bare name, a name carrying a range to match, a path scoping the pin to one parent, or `$name` to reuse your own dependency's range. Documented by [npm](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#overrides).
...
The same pins in Yarn's spelling, taking the same selectors. Both fields are read, and `overrides` wins a conflict — so a repo that declares a pin in both for portability stays silent. Documented by [Yarn](https://yarnpkg.com/configuration/manifest#resolutions).
...
Apply a patch file to a package's contents, keyed by name and version. Running `nub patch <pkg>` writes both the patch and this entry for you. A key matching no installed package fails the install unless `allowUnusedPatches` downgrades it to a warning. Documented by pnpm: [patchedDependencies](https://pnpm.io/cli/patch#patcheddependencies), [allowUnusedPatches](https://pnpm.io/cli/patch#allowunusedpatches).
...
Monorepo member globs. Members resolve to each other through a `workspace:` specifier instead of the registry. Documented by [npm](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#workspaces).
...
Name a version once and let every member reference it, so a bump is a one-line edit. The default catalog is `catalog:`; named catalogs are `catalog:<name>`. A reference naming no defined entry is an error rather than a fallback, so a typo fails the install. Documented by [Bun](https://bun.com/docs/install/catalogs).
...
Which dependencies may run install scripts — see [lifecycle scripts](#lifecycle-scripts) for the deny-by-default posture around it. A registry dependency is keyed by name, anything else by its full specifier, and an explicit `false` always wins. Documented by [pnpm](https://pnpm.io/settings/build#allowbuilds).
...
Per-dependency flags. Setting `injected` hard-copies a workspace dependency instead of linking it, and `built: false` denies that package's install scripts. Documented by [pnpm](https://pnpm.io/package_json#dependenciesmeta).
...
Mark one of your own peer dependencies optional, so a consumer that doesn't install it is not an error. Documented by [npm](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#peerdependenciesmeta).
...
Which deprecated versions may resolve without a warning, by package and range. Documented by [pnpm](https://pnpm.io/settings/dependency-resolution#alloweddeprecatedversions).
...
The Node and package-manager versions the project requires. Nub provisions the Node named here, and `engineStrict` decides whether a mismatch warns or fails. Documented by [npm](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#engines).
...
| Key | Reference |
|---|---|
| `resolutionMode` | [pnpm](https://pnpm.io/settings/other#resolutionmode) |
| `autoInstallPeers` | [pnpm](https://pnpm.io/settings/peer-dependencies#autoinstallpeers) |
| `strictPeerDependencies` | [pnpm](https://pnpm.io/settings/peer-dependencies#strictpeerdependencies) |
| `dedupePeerDependents` | [pnpm](https://pnpm.io/settings/peer-dependencies#dedupepeerdependents) |
| `resolvePeersFromWorkspaceRoot` | [pnpm](https://pnpm.io/settings/peer-dependencies#resolvepeersfromworkspaceroot) |
| `linkWorkspacePackages` | [pnpm](https://pnpm.io/workspaces#linkworkspacepackages) |
| `saveWorkspaceProtocol` | [pnpm](https://pnpm.io/workspaces#saveworkspaceprotocol) |
| `savePrefix` | [pnpm](https://pnpm.io/settings/other#saveprefix) |
| `saveExact` | [npm](https://docs.npmjs.com/cli/v11/using-npm/config#save-exact) |
| `supportedArchitectures` | [pnpm](https://pnpm.io/settings/dependency-resolution#supportedarchitectures) |
| `ignoredOptionalDependencies` | [pnpm](https://pnpm.io/settings/dependency-resolution#ignoredoptionaldependencies) |
| `minimumReleaseAge` | [pnpm](https://pnpm.io/settings/dependency-resolution#minimumreleaseage) |
| `minimumReleaseAgeExclude` | [pnpm](https://pnpm.io/settings/dependency-resolution#minimumreleaseageexclude) |
| `engineStrict` | [pnpm](https://pnpm.io/settings/cli#enginestrict) |
| `registry`, `@scope:registry` | [npm](https://docs.npmjs.com/cli/v11/using-npm/config#registry) |
