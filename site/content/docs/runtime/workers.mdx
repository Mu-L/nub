...
**Cloned `Blob` identity.** A `Blob` survives `postMessage` / `structuredClone` with its bytes and a `Blob` constructor name intact, but `cloned instanceof Blob` is `false`: nub replaces the global `Blob` with a transparent subclass to support `blob:` workers, while the structured-clone deserializer produces a native `node:buffer` Blob. The data round-trips correctly; only the identity check against the global differs.
