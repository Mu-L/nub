...
- **A pull request with no checks is UNVERIFIED, and the tooling says so.** `scripts/ci-watch.ts` and `scripts/merge-cascade.ts` both refuse to call an empty or all-skipped rollup green — a skipped check ran nothing, so it verifies nothing. Never read "no checks reported" as "nothing to wait for".
