...
The CSPRNG line is avoidable. `InitializeOncePerProcessInternal` calls `ncrypto::CSPRNG(nullptr, 0)` to confirm OpenSSL's random source is seeded, and installs a V8 entropy source that also goes through OpenSSL, so the first `RAND_status()` of the process happens before V8 starts and instantiates the DRBG, which activates the default provider and constructs its algorithm and name tables (`ossl_method_construct`, `ossl_namemap_stored`). V8 only uses that entropy for hash seeds, address-space randomization and `Math.random()`, none of them cryptographic; the 2022 change that let V8 fall back to its own source ([#44493][pr44493]) already records that V8's entropy is proper on every platform. A build that seeds V8 from `uv_random()` (the OS CSPRNG: `getrandom`, `arc4random_buf`, `BCryptGenRandom`) and keeps the eager seeding check only when FIPS is requested moves the `nodeStart → v8Start` span from 2.37 to 1.62 ms, takes `node -e 0` from 18.35 to 17.73 ms min and `hello.js` from 20.29 to 19.43 ms, removes `RAND_status` and the provider construction from the startup profile entirely (the OpenSSL config load stays, as it must), and passes the crypto random, FIPS, prime, secure-heap and hash-seed tests; the first `crypto.randomBytes()` then instantiates the DRBG lazily in 0.19 ms.
...
- **rapidhash secret generation in V8** — required by Node's [CVE-2026-21717][hashdos2026] fix, so the define Chrome uses is not available; measured above at 2–3 ms of every isolate start on Node 25+ (Deno pays it too). The cost is the schoolbook `mul_mod` inside `third_party/rapidhash-v8/secret.h`; a 128-bit multiply gives identical secrets 17× faster and, built into Node, 1.7 ms off `node -e 0` on Linux. No V8 CL or Node issue exists for it yet.
- **V8's entropy source routed through OpenSSL** — no issue; seeding V8 from `uv_random()` and deferring the CSPRNG check to FIPS builds is 0.7 ms per process on Linux, measured above, with every crypto and hash test passing.
