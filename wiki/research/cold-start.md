...
The two Linux-verified fixes were then built on the same macOS 15 arm64 runner (main, then the `mul_mod` change, then the entropy change, as incremental rebuilds of one tree) and measured interleaved on one Mac: hyperfine min / median of 100 runs, the interposer's in-process bucket as a min of 40 spawns, `performance.nodeTiming` as a min of 20, and ten `worker_threads` Workers spawned in one process.
