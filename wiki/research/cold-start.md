...
A proposal to remove CoreFoundation ([#44715][issue44715]) was closed in 2022 after screenshots on macOS 12.6 showed Instruments' Heap Allocations recorder no longer working without the framework loaded; nothing in that thread concerns ICU. Re-tested on macOS 15.7.9 and 26.5.2: malloc stack logging records the same 80,000 call stacks with the same symbolicated frames whether CoreFoundation is linked eagerly, delay-loaded or not linked, with the eager build as the positive control, and the Allocations instrument records all three. Apple's linker has shipped `-delay_framework` since Xcode 15, within Node's Xcode 16.4 floor, so the framework can be bound on first use with two link flags and no shim; a small binary saves 0.8–1.0 ms per start that way and pays about 1 ms on the first call that does use the framework (`--use-system-ca`, `fs.watch`). From Daniel Lemire in [#180][perf180]:
