...
On Apple Silicon the generator fix is 1.4 ms of the process and the entropy change 0.6 ms; `--version` does not move with either, since it exits before OpenSSL is initialized and never creates an isolate. **Both savings are once per process, not per isolate.** V8 writes the generated secret into the read-only roots, which are shared across the isolates of a process, so only the first isolate generates one: `HashSeed::InitializeRoots` runs from the read-only heap setup and the shared-heap deserializer records that the seed is already initialized by then (`src/snapshot/shared-heap-deserializer.cc`). A four-way build measured on Linux on 2026-09-19 (below) finds no per-Worker effect from the generator fix, 0.02 ms with a 95% interval of -0.01 to 0.06 ms, so the 0.85 ms per-Worker delta in the table above is not the generator cost and does not reproduce.

**Both fixes re-measured together, one box, one session (2026-09-19).** The earlier per-fix figures came from different sessions on different machines, which is why they did not add up. Node `main` at bca9bbee9 was built four ways from one tree on a 32-vCPU AMD EPYC 7B13 (gcc 13, OpenSSL 3.5.8): both fixes off, each alone, and both on, with the entropy fix removed by reverse-applying its own commit and the generator fix applied from the landed V8 commit. The four binaries ran interleaved in shuffled order, 1,500 rounds per workload, medians with bootstrap 95% intervals over rounds. The generator fix saves 2.56 ms (2.47 to 2.64) on `node -e 0` and 2.61 ms (2.52 to 2.70) on `hello.js`; the entropy fix saves 0.84 ms (0.73 to 0.93) and 0.72 ms (0.61 to 0.84); together 3.37 ms (3.28 to 3.45) and 3.39 ms (3.30 to 3.48). The two are independent: the interaction term is 0.04 ms (-0.06 to 0.12). Each lands in its own startup stage, the entropy fix taking `nodeStart → v8Start` from 2.64 to 1.89 ms and the generator fix taking `v8Start → environment` from 8.71 to 6.46 ms, with neither moving the other's stage. Worker spawn to `online` does not move: 12.33 ms against 12.23 ms with both fixes. The generator saving here is larger than the 1.7 ms measured earlier on an Intel box, which is expected for a change that removes 64-bit divisions, and a minimum-of-N comparison is the wrong instrument for a saving this size: on synthetic data with a planted 1.0 ms effect the difference of minima ranged from 0.60 to 1.15 ms while the difference of medians held within 0.1 ms.
...
- 2026-09-19 — **REVERSAL:** the generator fix is once per PROCESS, not per isolate, and it does not speed up Worker spawns. The doc previously said 1.4 ms per isolate including 0.85 ms per Worker spawn. V8 writes the generated secret into the shared read-only roots, so only the first isolate of a process generates one, and a four-way build measured on Linux finds the per-Worker effect at 0.02 ms (-0.01 to 0.06). That measurement also replaces the per-fix figures, which had come from different sessions on different machines: on one AMD EPYC 7B13 box, medians over 1,500 interleaved rounds, the generator fix is 2.56 ms of `node -e 0` and the entropy fix 0.84 ms, with an interaction of 0.04 ms, so they add to 3.37 ms.
