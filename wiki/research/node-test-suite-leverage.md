...
The `mod.rs` entrypoint reads the config, walks `runner/suite/test/` recursively, and runs each `.js` through Deno or skips per the entry. CI fails when an expected-pass entry regresses, or an unlisted file passes and should be promoted. The viewer ([`denoland/node_test_viewer`](https://github.com/denoland/node_test_viewer), TS, last push 2026-05-14) ingested the daily JSON output and rendered a per-module table (`fs(358)`, `http(437)`, `stream(213)`, …) at `node-test-viewer.deno.dev/results/latest`. That host 404s as of 2026-08-20.
...
- [denoland/node_test_viewer](https://github.com/denoland/node_test_viewer) — viewer source (last push 2026-05-14, not archived). Its `node-test-viewer.deno.dev` deployment 404s as of 2026-08-20.
...
- 2026-08-20 — Delinked the `node-test-viewer.deno.dev` citations: the deployment 404s (checked 2026-08-20), though `denoland/node_test_viewer` itself is live and unarchived. Corrected its last-push date to 2026-05-14.
