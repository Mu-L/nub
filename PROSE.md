...
- **A review reply reads like a person typing one, not a report.** Sentence fragments are the normal register ("Haven't investigated that.", "Not tested on AIX.", "Fixed."). Open on the fact. A short sentence that answers the question in the abstract before the fact ("Not before opening, and it was broken.") is preamble and a giveaway; delete it and start with what happened ("Broken in the first push. `sha256` failed with …"). Never open with an affirmation and a colon ("Right:", "Yes:", "Correct:") — say "Yes, …" or just state the fact. One colon per comment at most; the rest become a period or a "so"/"because". Perfect grammar in every sentence is itself a tell, and an occasional typo is not something to scrub out of a comment — leave it, or let one in. Identifiers, commands, commit messages and docs stay exact.
...
| Affirmation-plus-colon opener | Right: `conf_file` is null then, and OpenSSL still reads the file. | Yes, `conf_file` is null then and OpenSSL still reads the file. |
| Colon doing every join | The flag depends on it: the explicit load disables the fallback: nothing activates the default provider first. | The flag depends on it. The explicit load disables the fallback, so nothing activates the default provider first. |
...
- **Perfect grammar, uniform polish.** Every sentence complete, every clause balanced, no slip anywhere. A person replying in a thread drops subjects ("Haven't investigated that."), leaves fragments, and lets the occasional typo stand. Fix: in a reply or a chat-register comment, keep the fragments and do not scrub every slip; polish every sentence only where the surface demands it (docs, commit messages, identifiers, commands).
- **The colon as the universal join.** A colon after every claim, one opening the reply ("Right:", "Yes:"), one before each list of consequences — the fact→pivot→significance shape with the dash swapped out. Fix: ration colons to about one per comment or paragraph; a period, a "so", or a "because" carries the rest, and an affirmation never takes a colon.
...
- **Code comments are prose and get the same pass.** A comment states the invariant, the design or the reason in the fewest words. No colon chains, no narrated structure ("Note that…", "This is because…"), no restating the code, no shaped closer. A comment that reads like a paragraph from a PR description is too long.
