...
**Local host won't show the behavior?** If the thing you need to probe is OS/platform-gated — macOS Seatbelt / `sandbox-exec` / codesigning, Windows cmd.exe / the shell-emulator / Authenticode, musl/glibc, a Node floor — this local loop can't reach it. Linux corners go to Docker (AGENTS.md's Docker section); a real **macOS or Windows** behavior goes to the **`ci-adhoc-test` skill** — a self-contained probe under `tests/<probe>/` run by a branch-scoped workflow with **no PR required**.
