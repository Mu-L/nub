...
2. **Run it on a quiet machine.** The dev Mac is never quiet; a runtime benchmark goes to an idle spot VM through `remote-build --job adhoc` (the script runs at the repo root with `NUB_BIN` set). For an install benchmark, `uptime` first and follow the `benchmarking` skill's load gate.
