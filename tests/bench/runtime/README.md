...
| `pool-bound.sh` | The same pool on workloads heavy enough to be bound by it: bcrypt, scrypt and pbkdf2 logins, sharp thumbnails and gzip of a JSON response, `node` against `nub`. Meant for a box with more than 4 cores (`--machine c4-standard-16`); on 4 cores or fewer Nub sets nothing different. |
