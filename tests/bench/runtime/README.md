...
| `pool-bound.sh` | The same pool on workloads heavy enough to be bound by it: bcrypt, scrypt and pbkdf2 logins, and sharp thumbnails, `node` against `nub`. Meant for a many-core box (`--machine c3-standard-22`); on 4 cores or fewer Nub sets nothing different. |
